...
function AssignAssetEdit() {
  ...
  const vendordata = async () => {
    try {
      const response = await fetch("http://127.0.0.1:3001/api/v1/vendors");
  ...
  const fetchAssignAssetData = async () => {
    try {
      const response = await fetch(
        `http://127.0.0.1:3001/api/v1/assign_assets/${params.id}`
      );
      const data = await response.json();
      setId(data.id);
  ...
  const handleEditSubmit = (e) => {
    ...
    fetch(`http://127.0.0.1:3001/api/v1/assign_assets/${id}`, {
      method: "PUT",
  ...
  const producttypedata = async () => {
    try {
      const response = await fetch("http://127.0.0.1:3001/api/v1/producttype");
      const data = await response.json();
      setProductType(data);
    } catch (error) {
      console.error("Error fetching vendors:", error);
